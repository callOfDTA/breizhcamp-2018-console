...
export default class Service {
  ...
  chercherSessions(motCle: string) {
    let talksTrouver: Session[] = [];

    return new Promise<Session[]>((resolve, reject) => {
      this.talks.forEach((elem: Session) => {
        if (elem.titre.toUpperCase().includes(motCle.toUpperCase()))
          talksTrouver.push(elem);
      });
      if (talksTrouver.length != 0) {
        console.log("JE NE SUIS PAS EN ERREUR")
        resolve(talksTrouver);
      } else {
